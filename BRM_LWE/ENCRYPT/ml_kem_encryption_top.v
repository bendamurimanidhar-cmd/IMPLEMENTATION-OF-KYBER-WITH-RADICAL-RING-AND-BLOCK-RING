...
module mlkem_encrypt_top #(
    ...
    wire [7:0] rr_read_idx = (j_idx_reg >= i_idx_reg) ? (j_idx_reg - i_idx_reg) : (j_idx_reg + K_PARAM - i_idx_reg);
    wire is_in_k0 = (j_idx_reg < 2) && (i_idx_reg < 2);
    wire [7:0] brm_read_idx = is_in_k0 ? 
        ((j_idx_reg >= i_idx_reg) ? (j_idx_reg - i_idx_reg) : (j_idx_reg + 2 - i_idx_reg)) : 
        (j_idx_reg * K_PARAM + i_idx_reg);
    wire [7:0] read_poly_idx = IS_RRLWE ? rr_read_idx :
                               (IS_BRMLWE ? brm_read_idx : (j_idx_reg * K_PARAM + i_idx_reg));
    wire [11:0] base_idx_write = {4'd0, write_poly_idx} * 12'd256;
    wire [11:0] base_idx_read  = {4'd0, read_poly_idx} * 12'd256;
    wire [11:0] base_idx = (current_state == STATE_PROCESS_REJ) ? base_idx_write : base_idx_read;
    ...
    wire rr_twist = (j_idx_reg < i_idx_reg);
    wire brm_twist = is_in_k0 && (j_idx_reg < i_idx_reg);
        ...
        if (!rst_n) begin
            ...
            case (current_state)
                ...
                STATE_PROCESS_REJ: begin
                    ...
                    if (hit_256) begin
                        if ((j_idx_reg == K_LAST) || IS_RRLWE) begin
                            j_idx_reg <= 8'd0;

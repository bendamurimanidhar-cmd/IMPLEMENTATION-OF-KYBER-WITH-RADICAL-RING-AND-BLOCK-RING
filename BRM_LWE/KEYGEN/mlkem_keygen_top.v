...
module mlkem_keygen_top #(
    parameter K = 3,
    parameter K_PARAM = 3,
    parameter IS_RRLWE = 0,
    parameter IS_BRMLWE = 1
)(
    input  wire          clk,
    input  wire          rst_n,
    input  wire          start,
    ...
    wire [7:0] write_poly_idx = (i_idx_reg * K_PARAM + j_idx_reg);
    wire [7:0] rr_read_idx = (i_idx_reg >= j_idx_reg) ? (i_idx_reg - j_idx_reg) : (i_idx_reg + K_PARAM - j_idx_reg);
    wire is_in_k0 = (i_idx_reg < 2) && (j_idx_reg < 2);
    wire [7:0] brm_read_idx = is_in_k0 ? 
        ((i_idx_reg >= j_idx_reg) ? (i_idx_reg - j_idx_reg) : (i_idx_reg + 2 - j_idx_reg)) : 
        (i_idx_reg * K_PARAM + j_idx_reg);
    wire [7:0] read_poly_idx = IS_RRLWE ? rr_read_idx :
                               (IS_BRMLWE ? brm_read_idx : (i_idx_reg * K_PARAM + j_idx_reg));
    wire [MAT_AW-1:0] base_idx_write = write_poly_idx * N;
    wire [MAT_AW-1:0] base_idx_read  = read_poly_idx * N;
    ...
    assign bram_A_addrA = (current_state == STATE_PROCESS_REJ) ? (base_idx_write + coeff_counter) : (base_idx_read + dot_addr_a0);
    ...
    assign bram_A_addrB = (current_state == STATE_PROCESS_REJ) ? (base_idx_write + coeff_counter + 10'd1) : (base_idx_read + dot_addr_a1);
    ...
    wire rr_twist = (i_idx_reg < j_idx_reg);
    wire brm_twist = is_in_k0 && (i_idx_reg < j_idx_reg);
    wire twist_enable = IS_RRLWE ? rr_twist : (IS_BRMLWE ? brm_twist : 1'b0);
    ntt_dot_prod #(.Q(3329), .IS_RRLWE(IS_RRLWE), .IS_BRMLWE(IS_BRMLWE)) u_dot (
        .clk        (clk),
        .rst_n      (rst_n),
        .start      (dot_start),
        .done       (dot_done),
        ...
        if (!rst_n) begin
            ...
            case (current_state)
                ...
                STATE_PROCESS_REJ: begin
                    ...
                    if (hit_256) begin
                        if ((j_idx_reg == (K_PARAM - 8'd1)) || IS_RRLWE) begin
                            j_idx_reg <= 8'd0;
                            ...
                            if (IS_BRMLWE && (i_idx_reg < 2) && (j_idx_reg == 0)) begin
                                j_idx_reg <= 8'd2; 
                            end else begin
